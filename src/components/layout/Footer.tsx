  ...
  <footer className="bg-foreground text-background">
    ...
    <div className="container-wide section-padding">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 lg:gap-12">
        ...
        <div className="col-span-2 mb-4 lg:mb-0">
          <Link href="/" className="flex items-center">
            <Image src="/logo-rocket4rpo-white.webp" alt="Rocket4RPO — experts Talent Acquisition intégrés" width={384} height={256} sizes="(max-width: 768px) 120px, 144px" className="h-20 md:h-24 w-auto" />
          </Link>
          <p className="mt-4 text-sm text-background/75 leading-relaxed max-w-xs">
            Talent Acquisition experts intégrés à vos équipes pour accélérer vos recrutements.
          </p>
          <div className="mt-4 space-y-2 text-xs text-background/70">
      ...
      <div className="mt-16 pt-8 border-t border-background/10 flex flex-col md:flex-row items-center justify-between gap-4">
        <p className="text-xs text-background/70">© {new Date().getFullYear()} Rocket4RPO. Tous droits réservés.</p>
        <div className="flex items-center gap-6">
          <a href="https://www.linkedin.com/company/rocket4rpo" target="_blank" rel="noopener noreferrer" className="text-background/70 hover:text-primary transition-colors text-sm">
            LinkedIn
          </a>
          <a href="https://www.rocket4sales.com" target="_blank" rel="noopener noreferrer" className="text-background/70 hover:text-primary transition-colors text-sm">
            Rocket4Sales
          </a>
          <Link href="/sitemap.xml" className="text-background/70 hover:text-primary transition-colors text-sm">
            Sitemap
          </Link>
        </div>
      </div>
